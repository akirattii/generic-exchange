...
module.exports = class GenericExchange {
  constructor({
    ...
    // Prepares DB Connections for Transaction process, actually there is only one instance named "generic".
    // eg: {"generic":<connection>}
    self.txConnections = {};
    return cb && cb(null);
  }
  ...
  getTxConnection(conName, cb) {
    const self = this;
    if (self.debugLevel >= DEBUG_LEVEL) self.logger.info(`[GEX] #getTxConnection: Getting a connection named: "${conName}"`);
    const con = self.txConnections[conName];
    // Check whether the connection is alive
    if (!con || con.state === "disconnected") {
      // the connection is dead, so re-connect!
      self.dao.getTxConnection((err, newcon) => {
        if (err) return _cb && _cb(err);
        self.txConnections[conName] = newcon;
        if (self.debugLevel >= DEBUG_LEVEL)
          self.logger.info(`[GEX] #createTxConnections: new connection named "${conName}" created!`);
        return cb && cb(null, newcon);
      });
    } else {
      // the connection is alive, so returns current connection.
